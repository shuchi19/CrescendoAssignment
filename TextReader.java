...
public class TextReader extends FilterReader {
	...
	// First, define the exceptions that can be thrown by this class. These
	// exceptions
	// are subclasses of RuntimeException, so they do not require mandatory
	// error-handling.
	// Also, if you prefer, you can turn off excpetions by calling the IOCheck()
	// method.
	// In that case, when an exception occurs during processing by one of the
	// methods
	...
	// getAnyChar() -- reads and returns the next character in the stream, even if
	// it
	...
	// getChar(), getByte(), etc. -- skip over whitespace characters, then try to
	// read a
	...
	// getlnChar(), getlnByte() etc. -- Work the same as getChar(), etc., except
	// that
	// after the value is read, any other characters on the same line, including the
	// end-of-line, are read and discarded.

	// eoln() -- this first reads and discards any spaces and tabs. Then tests
	// whether
	...
	// eof() -- this first reads and discards any spaces, ends-of-line and tabs.
	// Then tests
	// whether the next thing in the stream is the end-of-file. If you want to test
	// for
	// eof without discarding spaces, ends-of-line, and tabs, check whether peek()
	// == '\0'.

	// skipWhiteSpace() -- reads and discards any whitespace characters (spaces,
	// tabs, and
	// end-of-lines). Stops when the next character is a non-whitespace character or
	// is eof.

	// skipNonLetters() -- reads and discards any non-letters, stopping when the
	// next character
